...
# Get a summary of utilized resources during the job
sacct -j $SLURM_JOB_ID --format=JobID,MaxRSS,MaxVMSize,Elapsed >> resources_$SLURM_JOB_ID.txt
```
...
De même, la dernière ligne de l'exemple de script BASH sert à générer un fichier contenant les informations suivantes :

* `JobID` : L'identifiant de job attribué à notre processus.
* `MaxRSS` : La quantité maximale de RAM physique utilisée.
* `MaxVMSize` : La quantité maximale de mémoire virtuelle utilisée (RAM + swap + mémoire attribuée mais non utilisée).
* `Elapsed` : Temps total d'exécution du processus, à l'exclusion du temps d'attente en file.

Ces informations seront enregistrées dans un fichier nommé *resources_$SLURM_JOB_ID.txt*, où `$SLURM_JOB_ID` sera remplacé par l'identifiant de job attribué à notre processus.

Si, pour une raison quelconque, nous devons relancer le processus, il est recommandé de prendre en compte les valeurs d'utilisation des ressources obtenues lors d'une exécution précédente comme guide pour définir les paramètres dans notre script BASH, afin d'éviter de réserver plus de ressources que nécessaire. 
:::  
...
Similarly, the last line of the BASH script example serves to generate a file that will contain the following information:

* `JobID`: The Job-ID assigned to our process.
* `MaxRSS`: The maximum amount of physical RAM used.
* `MaxVMSize`: The maximum amount of virtual memory used (RAM + swap + assigned but unused memory).
* `Elapsed`: Total process execution time, excluding queuing time.

This information will be saved in a file named *resources_$SLURM_JOB_ID.txt*, where `$SLURM_JOB_ID` will be replaced by the Job-ID assigned to our process.

If, for any reason, we need to relaunch the process, it is recommended to consider the resource usage values obtained from a previous run as a guide for defining the parameters within our BASH script, thus avoiding the reservation of more resources than our process actually requires.
::: 
